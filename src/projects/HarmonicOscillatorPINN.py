...
GitHub: https://github.com/benmoseley/harmonic-oscillator-pinn/tree/main
...
# %% Define Oscillator and NN architecture
...
for i in range(20000):
    ...
    yhp = model(x_physics)
    dx = torch.autograd.grad(yhp, x_physics, torch.ones_like(yhp), create_graph=True)[
        0
    ]  # computes dy/dx
    dx2 = torch.autograd.grad(dx, x_physics, torch.ones_like(dx), create_graph=True)[
        0
    ]  # computes d^2y/dx^2
    physics = (
        dx2 + mu * dx + k * yhp
