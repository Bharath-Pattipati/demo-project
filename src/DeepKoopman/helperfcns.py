...
def stack_data(data, num_shifts, len_time):
    ...
    for j in np.arange(num_shifts + 1):
        for count in np.arange(num_traj):
            data_tensor_range = np.arange(
                count * new_len_time, new_len_time + count * new_len_time
            )
            data_tensor[j, data_tensor_range, :] = data[
                count * len_time + j : count * len_time + j + new_len_time, :
            ]
    ...
    if params["opt_alg"] == "adam":
        ...
        if params["decay_rate"] > 0:
            optimizer = tf.train.AdadeltaOptimizer(
                params["learning_rate"], params["decay_rate"]
            ).minimize(regularized_loss, var_list=trainable_var)
        else:
            # defaults 0.001, 0.95
            optimizer = tf.train.AdadeltaOptimizer(params["learning_rate"]).minimize(
                regularized_loss, var_list=trainable_var
            )
    elif params["opt_alg"] == "adagrad":
        # also has initial_accumulator_value parameter
        optimizer = tf.train.AdagradOptimizer(params["learning_rate"]).minimize(
            regularized_loss, var_list=trainable_var
        )
        ...
        optimizer = tf.train.AdagradDAOptimizer(
            params["learning_rate"], tf.get_global_step()
        ).minimize(regularized_loss, var_list=trainable_var)
        ...
        optimizer = tf.train.ProximalAdagradOptimizer(params["learning_rate"]).minimize(
            regularized_loss, var_list=trainable_var
        )
        ...
        if params["decay_rate"] > 0:
            ...
            optimizer = tf.train.RMSPropOptimizer(params["learning_rate"]).minimize(
                regularized_loss, var_list=trainable_var
            )
    else:
    ...
    if not params["been5min"]:
        ...
        if current_time - start > 5 * 60:
            if best_error > params["min_5min"]:
                print(
                    "too slowly improving in first five minutes: err %.15f" % best_error
                )
                ...
                print(
                    "been 5 minutes, err = %.15f < %.15f"
                    % (best_error, params["min_5min"])
                )
    ...
    if not params["been20min"]:
        ...
        if current_time - start > 20 * 60:
            if best_error > params["min_20min"]:
                print(
                    "too slowly improving in first 20 minutes: err %.15f" % best_error
                )
                ...
                print(
                    "been 20 minutes, err = %.15f < %.15f"
                    % (best_error, params["min_20min"])
                )
    ...
    if not params["been40min"]:
        ...
        if current_time - start > 40 * 60:
            if best_error > params["min_40min"]:
                print(
                    "too slowly improving in first 40 minutes: err %.15f" % best_error
                )
                ...
                print(
                    "been 40 minutes, err = %.15f < %.15f"
                    % (best_error, params["min_40min"])
                )
    ...
    if not params["been1hr"]:
        ...
        if current_time - start > 60 * 60:
            if best_error > params["min_1hr"]:
                ...
                print(
                    "been 1 hour, err = %.15f < %.15f" % (best_error, params["min_1hr"])
                )
                save_now = 1
    ...
    if not params["been2hr"]:
        ...
        if current_time - start > 2 * 60 * 60:
            if best_error > params["min_2hr"]:
                ...
                print(
                    "been 2 hours, err = %.15f < %.15f"
                    % (best_error, params["min_2hr"])
                )
                save_now = 1
    ...
    if not params["been3hr"]:
        ...
        if current_time - start > 3 * 60 * 60:
            if best_error > params["min_3hr"]:
                print(
                    "too slowly improving in first three hours: err %.15f" % best_error
                )
                ...
                print(
                    "been 3 hours, err = %.15f < %.15f"
                    % (best_error, params["min_3hr"])
                )
                save_now = 1
    ...
    if not params["been4hr"]:
        ...
        if current_time - start > 4 * 60 * 60:
            if best_error > params["min_4hr"]:
                print(
                    "too slowly improving in first four hours: err %.15f" % best_error
                )
                ...
                print(
                    "been 4 hours, err = %.15f < %.15f"
                    % (best_error, params["min_4hr"])
                )
                save_now = 1
    ...
    if not params["beenHalf"]:
        ...
        if current_time - start > params["max_time"] / 2:
            if best_error > params["min_halfway"]:
                ...
                print(
                    "Halfway through time, err = %.15f < %.15f"
                    % (best_error, params["min_halfway"])
                )
...
def save_files(sess, csv_path, train_val_error, params, weights, biases):
    ...
    for key, value in weights.items():
        np.savetxt(
            csv_path.replace("error", key), np.asarray(sess.run(value)), delimiter=","
        )
    ...
    print(
        "min train: %.12f, min val: %.12f, min reg. train: %.12f, min reg. val: %.12f"
        % (
            params["minTrain"],
            params["minTest"],
            params["minRegTrain"],
...
def set_defaults(params):
    ...
    if "exp_suffix" not in params:
        ...
        params["exp_suffix"] = "_" + datetime.datetime.now().strftime(
            "%Y_%m_%d_%H_%M_%S_%f"
        )
    ...
    if "num_complex_pairs" not in params:
        raise KeyError(
            "Error, must give number of pairs of complex eigenvalues: num_complex_pairs"
        )
    ...
    if "seed" not in params:
        ...
        print(
            "setting default: choosing random seed of %d and saving to params"
            % random_seed
        )
    ...
    if isinstance(params["dist_weights_omega"], str):
        params["dist_weights_omega"] = [params["dist_weights_omega"]] * (
            len(params["widths_omega_real"]) - 1
        )
    if isinstance(params["dist_biases_omega"], int):
        params["dist_biases_omega"] = [params["dist_biases_omega"]] * (
            len(params["widths_omega_real"]) - 1
        )
    ...
    if "num_steps_per_file_pass" not in params:
        print(
            "setting default: up to 1000000 steps per training file before moving to next one"
        )
    ...
    if "min_halfway" not in params:
        ...
        print(
            "setting default: must reach %f in first half of time allotted"
            % params["min_halfway"]
        )
